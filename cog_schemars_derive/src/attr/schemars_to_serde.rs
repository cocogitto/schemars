...
// List of keywords that can appear in #[serde(...)]/#[cog_schemars(...)] attributes which we want
...
pub(crate) static SERDE_KEYWORDS: &[&str] = &[
    ...
    // cog_schemars attr.
    "bound",
    // Special cases - `with`/`serialize_with` are passed to serde but not copied from cog_schemars
    ...
    // serialized. We also check the `with` value on cog_schemars/serde attrs e.g. to support deriving
...
// If a struct/variant/field has any #[cog_schemars] attributes, then create copies of them
...
fn process_attrs(ctxt: &Ctxt, attrs: &mut Vec<Attribute>) {
    ...
    *attrs = other_attrs;
    ...
    // Copy appropriate #[cog_schemars(...)] attributes to #[serde(...)] attributes
    let (mut serde_meta, mut cog_schemars_meta_names): (Vec<_>, HashSet<_>) =
        get_meta_items(attrs, "cog_schemars", ctxt)
            .into_iter()
            .filter_map(|meta| {
    ...
    if cog_schemars_meta_names.contains("skip") {
        cog_schemars_meta_names.insert("skip_serializing".to_string());
        cog_schemars_meta_names.insert("skip_deserializing".to_string());
    }

    // Re-add #[serde(...)] attributes that weren't overridden by #[cog_schemars(...)] attributes
    for meta in get_meta_items(&serde_attrs, "serde", ctxt) {
        if let Some(i) = get_meta_ident(&meta) {
            if !cog_schemars_meta_names.contains(&i)
...
mod tests {
    ...
    fn test_process_serde_attrs() {
        let mut input: DeriveInput = parse_quote! {
            ...
            #[cog_schemars(rename = "overriden", another_unknown_word)]
            #[misc]
            struct MyStruct {
                /// blah blah blah
                ...
                #[cog_schemars(with = "with", bound = "bound")]
                field2: i32,
                #[cog_schemars(skip)]
