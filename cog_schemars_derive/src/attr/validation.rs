...
use super::{
    parse_meta::{
        ...
        parse_cog_schemars_regex, parse_validate_regex, require_path_only, LengthOrRange,
    },
    AttrCtxt,
};
...
impl ValidationAttrs {
    ...
    fn add_mutators2(&self, mutators: &mut Vec<TokenStream>, mut_ref_schema: &TokenStream) {
        ...
        if let Some(regex) = self.regex.as_ref().or(self.pattern.as_ref()) {
            mutators.push(quote! {
                cog_schemars::_private::insert_validation_property(#mut_ref_schema, "string", "pattern", (#regex).to_string());
            });
        }

        if let Some(contains) = &self.contains {
            mutators.push(quote! {
                cog_schemars::_private::must_contain(#mut_ref_schema, &#contains.to_string());
            });
        }
        ...
        if let Some(inner) = &self.inner {
            ...
            if !inner_mutators.is_empty() {
                mutators.push(quote! {
                    cog_schemars::_private::apply_inner_validation(#mut_ref_schema, |inner_schema| { #(#inner_mutators)* });
                })
            }
        }
    }
    ...
    fn add_length_or_range(
        ...
        if let Some(min) = value.min.as_ref().or(value.equal.as_ref()) {
            ...
            mutators.push(quote!{
                cog_schemars::_private::insert_validation_property(#mut_ref_schema, #required_format, #key, #min);
            });
        }
        ...
        if let Some(max) = value.max.as_ref().or(value.equal.as_ref()) {
            ...
            mutators.push(quote!{
                cog_schemars::_private::insert_validation_property(#mut_ref_schema, #required_format, #key, #max);
            });
        }
    }
                ...
                match (&self.pattern, &self.regex, &self.contains) {
                    ...
                    (None, None, None) => {
                        if cx.attr_type == "validate" {
                            ...
                            self.regex = parse_cog_schemars_regex(meta, cx).ok()
                        }
                    }
                }
            }
    ...
    fn handle_format(&mut self, meta: Meta, format: Format, cx: &AttrCtxt) {
        match self.format {
            ...
            None => {
                ...
                // but restrict it to path only in cog_schemars attr.
