...
impl CommonAttrs {
    fn populate(
        &mut self,
        attrs: &[Attribute],
        cog_schemars_cx: &mut AttrCtxt,
        serde_cx: &mut AttrCtxt,
    ) {
        self.process_attr(cog_schemars_cx);
                        ...
                        if lit_str.parse::<Path>().is_ok() {
                            ...
                            cx.error_spanned_by(&expr, format_args!(
                                "`example` value must be an expression, and string literals that may be interpreted as function paths are currently disallowed to avoid migration errors \
                                 (this restriction may be relaxed in a future version of cog_schemars).\n\
                                If you want to use the result of a function, use `#[cog_schemars(example = {lit_str_value}())]`.\n\
                                Or to use the string literal value, use `#[cog_schemars(example = &\"{lit_str_value}\")]`."));
                        }
                    }
                    ...
                    }) = &expr
                    {
                        if lit_str.parse::<Expr>().is_ok() {
                            cx.error_spanned_by(
                                &expr,
                                format_args!(
                                    "Expected a `fn(&mut Schema)` or other value implementing `cog_schemars::transform::Transform`, found `&str`.\nDid you mean `#[cog_schemars(transform = {})]`?",
                                    lit_str.value()
                                ),
                            )
                        }
    ...
    pub fn add_mutators(&self, mutators: &mut Vec<TokenStream>) {
        ...
        if let Some(doc) = &self.doc {
            if title.is_none() || description.is_none() {
                mutators.push(quote!{
                    const title_and_description: (&str, &str) = cog_schemars::_private::get_title_and_description(#doc);
                });
        ...
        if let Some(title) = title {
            mutators.push(quote! {
                cog_schemars::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "title", #title);
            });
        }
        if let Some(description) = description {
            mutators.push(quote! {
                cog_schemars::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "description", #description);
            });
        }

        if self.deprecated {
            mutators.push(quote! {
                cog_schemars::_private::insert_metadata_property(&mut #SCHEMA, "deprecated", true);
            });
        }
        ...
        if !self.examples.is_empty() {
            let examples = self.examples.iter().map(|eg| {
                quote! {
                    cog_schemars::_private::serde_json::value::to_value(#eg)
                }
            });
            mutators.push(quote! {
                cog_schemars::_private::insert_metadata_property(&mut #SCHEMA, "examples", cog_schemars::_private::serde_json::Value::Array([#(#examples),*].into_iter().flatten().collect()));
            });
        }

        for (k, v) in &self.extensions {
            mutators.push(quote! {
                cog_schemars::_private::insert_metadata_property(&mut #SCHEMA, #k, cog_schemars::_private::serde_json::json!(#v));
            });
        }

        for transform in &self.transforms {
            mutators.push(quote! {
                cog_schemars::transform::Transform::transform(&mut #transform, &mut #SCHEMA);
            });
        }
    }
}
...
impl FieldAttrs {
    ...
    fn populate(&mut self, attrs: &[Attribute], cx: &Ctxt) {
        let cog_schemars_cx = &mut AttrCtxt::new(cx, attrs, "cog_schemars");
        ...
        self.common.populate(attrs, cog_schemars_cx, serde_cx);
        self.validation.populate(cog_schemars_cx, validate_cx, garde_cx);
        self.process_attr(cog_schemars_cx);
        self.process_attr(serde_cx);
    }
    ...
    fn process_meta(&mut self, meta: Meta, meta_name: &str, cx: &AttrCtxt) -> Option<Meta> {
        match meta_name {
            ...
            "schema_with" if cx.attr_type == "cog_schemars" => match self.with {
...
impl<'a> AttrCtxt<'a> {
    ...
    pub fn mutual_exclusive_error(&self, meta: &Meta, other_attr: &str) {
        if self.attr_type == "cog_schemars" {
            self.error_spanned_by(
                meta,
                format_args!(
                    "cog_schemars attribute cannot contain both `{}` and `{}`",
                    path_str(meta.path()),
                    other_attr,
                ),
            );
    ...
    pub fn duplicate_error(&self, meta: &Meta) {
        if self.attr_type == "cog_schemars" {
            self.error_spanned_by(
                meta,
                format_args!(
                    "duplicate cog_schemars attribute item `{}`",
                    path_str(meta.path())
                ),
            );
        }
        ...
        if self.attr_type == "cog_schemars" {
            for unhandled_meta in self.metas.iter().filter(|m| !is_known_serde_keyword(m)) {
                self.error_spanned_by(
                    unhandled_meta.path(),
                    format_args!(
                        "unknown cog_schemars attribute `{}`",
