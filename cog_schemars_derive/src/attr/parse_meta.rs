...
pub fn parse_length_or_range(outer_meta: Meta, cx: &AttrCtxt) -> Result<LengthOrRange, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta, cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            unknown => {
                if cx.attr_type == "cog_schemars" {
                    cx.error_spanned_by(
                        nested_meta,
                        format_args!(
                            "unknown item in cog_schemars {outer_name} attribute: `{unknown}`",
                        ),
                    );
                }
            }
...
pub fn parse_cog_schemars_regex(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta.clone(), cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            "path" => {
                cx.error_spanned_by(nested_meta, "`path` is not supported in `cog_schemars(regex(...))` attribute - use `cog_schemars(regex(pattern = ...))` instead")
            },
            unknown => {
                cx.error_spanned_by(
                    nested_meta,
                    format_args!("unknown item in cog_schemars `regex` attribute: `{unknown}`"),
                );
            }
        }
    }
    ...
    pattern.ok_or_else(|| {
        cx.error_spanned_by(
            outer_meta,
            "`cog_schemars(regex(...))` attribute requires `pattern = ...`",
        )
    })
}
...
pub fn parse_validate_regex(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta.clone(), cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            "pattern" => {
                cx.error_spanned_by(nested_meta, "`pattern` is not supported in `validate(regex(...))` attribute - use either `validate(regex(path = ...))` or `cog_schemars(regex(pattern = ...))` instead")
            },
            _ => {
...
pub fn parse_contains(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    let nested_meta_or_expr = match cx.attr_type {
        ...
        "cog_schemars" => parse_meta_list_with(&outer_meta, cx, ContainsFormat::parse),
        wat => {
            unreachable!("Unexpected attr type `{wat}` for `contains` item. This is a bug in cog_schemars, please raise an issue!")
        }
    }?;
        ...
        match path_str(nested_meta.path()).as_str() {
            ...
            unknown => {
                if cx.attr_type == "cog_schemars" {
                    cx.error_spanned_by(
                        nested_meta,
                        format_args!("unknown item in cog_schemars `contains` attribute: `{unknown}`"),
                    );
                }
            }
        }
