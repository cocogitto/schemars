...
pub fn expr_for_repr(cont: &Container) -> Result<SchemaExpr, syn::Error> {
    ...
    let mut schema_expr = SchemaExpr::from(quote!({
        let mut map = cog_schemars::_private::serde_json::Map::new();
        map.insert("type".into(), "integer".into());
        map.insert(
            "enum".into(),
            cog_schemars::_private::serde_json::Value::Array({
                let mut enum_values = cog_schemars::_private::alloc::vec::Vec::new();
                #(enum_values.push((#enum_ident::#variant_idents as #repr_type).into());)*
                enum_values
            }),
        );
        cog_schemars::Schema::from(map)
    }));
...
fn expr_for_field(field: &Field, is_internal_tagged_enum_newtype: bool) -> SchemaExpr {
    ...
    let schema_expr = if field.attrs.validation.required {
        quote_spanned! {span=>
            <#ty as cog_schemars::JsonSchema>::_cog_schemars_private_non_optional_json_schema(#GENERATOR)
        }
    } else if is_internal_tagged_enum_newtype {
        quote_spanned! {span=>
            cog_schemars::_private::json_schema_for_internally_tagged_enum_newtype_variant::<#ty>(#GENERATOR)
        }
    } else {
        quote_spanned! {span=>
    ...
    match with_attr {
        ...
        WithAttr::Function(fun) => {
            ...
            let type_def = quote_spanned! {fun.span()=>
                ...
                impl cog_schemars::JsonSchema for #ty_name {
                    fn always_inline_schema() -> bool {
                        true
                    }

                    fn schema_name() -> cog_schemars::_private::alloc::borrow::Cow<'static, str> {
                        cog_schemars::_private::alloc::borrow::Cow::Borrowed(#fn_name)
                    }

                    fn schema_id() -> cog_schemars::_private::alloc::borrow::Cow<'static, str> {
                        cog_schemars::_private::alloc::borrow::Cow::Borrowed(::core::concat!(
                    ...
                    fn json_schema(generator: &mut cog_schemars::SchemaGenerator) -> cog_schemars::Schema {
                        #fun(generator)
                    }
                }
            };
...
fn expr_for_enum(variants: &[Variant], cattrs: &serde_attr::Container) -> SchemaExpr {
    if variants.is_empty() {
        return quote!(cog_schemars::Schema::from(false)).into();
    }
    ...
    schemas.extend(complex_variants.into_iter().map(|variant| {
        ...
        let mut schema_expr =
            SchemaExpr::from(if variant.is_unit() && variant.attrs.with.is_none() {
                quote! {
                    cog_schemars::_private::new_unit_enum_variant(#name)
                }
            } else {
                let sub_schema = expr_for_untagged_enum_variant(variant, deny_unknown_fields);
                quote! {
                    cog_schemars::_private::new_externally_tagged_enum_variant(#name, #sub_schema)
                }
            });
    ...
    let variant_schemas = variants
        .map(|variant| {
            ...
            schema_expr.mutators.push(quote!(
                cog_schemars::_private::apply_internal_enum_variant_tag(&mut #SCHEMA, #tag_name, #name, #deny_unknown_fields);
            ));
...
fn expr_for_adjacent_tagged_enum<'a>(
    ...
    let schemas = variants
        .map(|variant| {
            ...
            let mut outer_schema = SchemaExpr::from(quote!(cog_schemars::json_schema!({
                "type": "object",
                "properties": {
                    #tag_name: (#tag_schema),
...
fn expr_for_internal_tagged_enum_variant(
    ...
    if let Some(with_attr) = &variant.attrs.with {
        let (ty, type_def) = type_for_schema(with_attr);
        let mut schema_expr = SchemaExpr::from(quote_spanned! {variant.original.span()=>
            <#ty as cog_schemars::JsonSchema>::json_schema(#GENERATOR)
        });
...
fn expr_for_tuple_struct(fields: &[Field]) -> SchemaExpr {
    ...
    quote!({
        let mut prefix_items = cog_schemars::_private::alloc::vec::Vec::new();
        #(#fields)*
        let len = cog_schemars::_private::serde_json::Value::from(prefix_items.len());
...
fn expr_for_struct(
        ...
        .map(|field| {
            if field.serde_attrs.flatten() {
                ...
                let mut schema_expr = SchemaExpr::from(quote_spanned! {ty.span()=>
                    cog_schemars::_private::json_schema_for_flatten::<#ty>(#GENERATOR, #required)
                });

                schema_expr.definitions.extend(type_def);

                field.with_contract_check(quote! {
                    cog_schemars::_private::flatten(&mut #SCHEMA, #schema_expr);
                })
            } else {
                let name = field.name();
                let (ty, type_def) = type_for_field_schema(field);
                ...
                let is_optional = if has_skip_serialize_if && has_default {
                    ...
                    quote!(if #GENERATOR.contract().is_serialize() {
                        ...
                        #has_default || (!#required_attr && <#ty as cog_schemars::JsonSchema>::_cog_schemars_private_is_option())
                    })
                };
                ...
                if let Some(default) = field_default_expr(field, set_container_default.is_some()) {
                    schema_expr.mutators.push(quote! {
                        #default.and_then(|d| cog_schemars::_cog_schemars_maybe_to_value!(d))
                            .map(|d| cog_schemars::_private::insert_metadata_property(&mut #SCHEMA, "default", d));
                    })
                }
                ...
                field.with_contract_check(quote!({
                    #type_def
                    cog_schemars::_private::insert_object_property(&mut #SCHEMA, #name, #is_optional, #schema_expr);
                }))
            }
            })
        .collect();
    ...
    SchemaExpr {
        ...
        creator: quote!(cog_schemars::json_schema!({
            "type": "object",
            #set_additional_properties
        })),
        mutators: properties,
