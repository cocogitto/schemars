...
#[proc_macro_derive(JsonSchema, attributes(cog_schemars, serde, validate, garde))]
...
#[proc_macro_derive(JsonSchema_repr, attributes(cog_schemars, serde))]
...
fn derive_json_schema(mut input: syn::DeriveInput, repr: bool) -> syn::Result<TokenStream> {
    ...
    if let Some(transparent_field) = cont.transparent_field() {
        ...
        return Ok(quote! {
            const _: () = {
                ...
                impl #impl_generics cog_schemars::JsonSchema for #type_name #ty_generics #where_clause {
                    fn always_inline_schema() -> bool {
                        <#ty as cog_schemars::JsonSchema>::always_inline_schema()
                    }

                    fn schema_name() -> cog_schemars::_private::alloc::borrow::Cow<'static, str> {
                        <#ty as cog_schemars::JsonSchema>::schema_name()
                    }

                    fn schema_id() -> cog_schemars::_private::alloc::borrow::Cow<'static, str> {
                        <#ty as cog_schemars::JsonSchema>::schema_id()
                    }
                    ...
                    fn _cog_schemars_private_non_optional_json_schema(#GENERATOR: &mut cog_schemars::SchemaGenerator) -> cog_schemars::Schema {
                        <#ty as cog_schemars::JsonSchema>::_cog_schemars_private_non_optional_json_schema(#GENERATOR)
                    }

                    fn _cog_schemars_private_is_option() -> bool {
                        <#ty as cog_schemars::JsonSchema>::_cog_schemars_private_is_option()
                    }
                };
            };
        });
        ...
        || (cont.attrs.is_renamed && !schema_base_name.contains('{'))
    {
        (
            quote! {
                cog_schemars::_private::alloc::borrow::Cow::Borrowed(#schema_base_name)
            },
            quote! {
        ...
        (
            quote! {
                cog_schemars::_private::alloc::borrow::Cow::Owned(
                    cog_schemars::_private::alloc::format!(
                        #schema_name_fmt
                        #(,#type_params=#type_params::schema_name())*
                        #(,#const_params=cog_schemars::_private::alloc::string::ToString::to_string(&#const_params))*)
                )
            },
            quote! {
                cog_schemars::_private::alloc::borrow::Cow::Owned(
        ...
        (
            quote! {
                cog_schemars::_private::alloc::borrow::Cow::Owned(
                    cog_schemars::_private::alloc::format!(#schema_name_fmt #(,#type_params::schema_name())* #(,#const_params)*)
                )
            },
            quote! {
                cog_schemars::_private::alloc::borrow::Cow::Owned(
    ...
    if let Some(bounds) = cont.serde_attrs.ser_bound() {
        ...
        for param in &mut cont.generics.params {
            if let syn::GenericParam::Type(ref mut type_param) = *param {
                type_param.bounds.push(parse_quote!(cog_schemars::JsonSchema));
            }
        }
    }
}
