...
#[allow(dead_code)]
#[derive(JsonSchema)]
#[cog_schemars(rename = "ValidateAttrStruct")]
pub struct SchemarsAttrStruct {
    #[cog_schemars(range(min = 1.0, max = 100.0))]
    min_max: f32,
    #[cog_schemars(range(min = ONE, max = HUNDRED))]
    min_max2: f32,
    #[cog_schemars(regex(pattern = hello_regex()))]
    regex_str: String,
    #[cog_schemars(contains(pattern = "substring..."))]
    contains_str: String,
    #[cog_schemars(email)]
    email_address: String,
    #[cog_schemars(url)]
