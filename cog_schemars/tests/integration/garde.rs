...
#[allow(dead_code)]
#[derive(JsonSchema)]
#[cog_schemars(rename = "GardeAttrStruct")]
pub struct SchemarsAttrStruct {
    #[cog_schemars(range(min = 1.0, max = 100.0))]
    min_max: f32,
    #[cog_schemars(range(min = ONE as f32, max = HUNDRED as f32))]
    min_max2: f32,
    #[cog_schemars(pattern(r"^[Hh]ello"))]
    regex_str: String,
    #[cog_schemars(contains(concat!("sub","string...")))]
    contains_str: String,
    #[cog_schemars(email)]
    email_address: String,
    #[cog_schemars(url)]
    homepage: String,
    #[cog_schemars(length(min = ONE, max = HUNDRED))]
    non_empty_str: String,
    #[cog_schemars(length(equal = 2), inner(length(min = 1)))]
    pair: Vec<String>,
    #[cog_schemars(required)]
    required_option: Option<bool>,
    #[cog_schemars(required)]
    #[serde(flatten)]
...
pub struct SchemarsAttrInner {
    #[cog_schemars(range(min = -100, max = 100))]
    x: i32,
}

#[test]
fn cog_schemars_attrs() {
