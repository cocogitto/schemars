...
pub fn json_schema_for_flatten<T: ?Sized + JsonSchema>(
    ...
    let mut schema = T::_cog_schemars_private_non_optional_json_schema(generator);

    if T::_cog_schemars_private_is_option() && !required {
...
#[doc(hidden)]
#[macro_export]
macro_rules! _cog_schemars_maybe_to_value {
    ($expression:expr) => {{
