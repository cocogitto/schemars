...
macro_rules! forward_impl {
    (($($impl:tt)+) => $target:ty) => {
        impl $($impl)+ {
            ...
            fn _cog_schemars_private_non_optional_json_schema(generator: &mut $crate::SchemaGenerator) -> $crate::Schema {
                <$target>::_cog_schemars_private_non_optional_json_schema(generator)
            }

            fn _cog_schemars_private_is_option() -> bool {
                <$target>::_cog_schemars_private_is_option()
            }
        }
    };
    ($ty:ty => $target:ty) => {
