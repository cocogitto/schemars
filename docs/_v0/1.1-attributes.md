...
[Validator](https://github.com/Keats/validator) allows setting `#[validate(...)]` attributes to restrict valid values of particular fields, many of which will be used by Schemars to generate more accurate schemas. These can also be overridden by `#[cog_schemars(...)]` attributes.
...
`#[validate(email)]` / `#[cog_schemars(email)]`<br />
`#[validate(email)]` / `#[cog_schemars(email)]`<br />
`#[validate(url)]` / `#[cog_schemars(url)]`
...
`#[validate(length(min = 1, max = 10))]` / `#[cog_schemars(length(min = 1, max = 10))]`<br />
`#[validate(length(equal = 10))]` / `#[cog_schemars(length(equal = 10))]`
...
`#[validate(range(min = 1, max = 10))]` / `#[cog_schemars(range(min = 1, max = 10))]`
...
`#[validate(regex = "path::to::regex")]` / `#[cog_schemars(regex = "path::to::regex")]`<br />
`#[cog_schemars(regex(pattern = r"^\d+$"))]`
...
`#[validate(contains = "string")]` / `#[cog_schemars(contains = "string")]`
...
`#[validate(required)]` / `#[cog_schemars(required)]`<br />
...
Set on a variant or field to generate this field's schema using the given function. This function must be callable as `fn(&mut cog_schemars::gen::SchemaGenerator) -> cog_schemars::schema::Schema`.
...
`#[cog_schemars(example = "some::function")]`
...
```rs
struct Struct {
    #[cog_schemars(inner(url, regex(pattern = "^https://")))]
    urls: Vec<String>,
}
```
