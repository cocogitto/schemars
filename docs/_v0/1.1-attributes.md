...
[Validator](https://github.com/Keats/validator) allows setting `#[validate(...)]` attributes to restrict valid values of particular fields, many of which will be used by Schemars to generate more accurate schemas. These can also be overridden by `#[cocogitto_schemars(...)]` attributes.
...
`#[validate(email)]` / `#[cocogitto_schemars(email)]`<br />
`#[validate(email)]` / `#[cocogitto_schemars(email)]`<br />
`#[validate(url)]` / `#[cocogitto_schemars(url)]`
...
`#[validate(length(min = 1, max = 10))]` / `#[cocogitto_schemars(length(min = 1, max = 10))]`<br />
`#[validate(length(equal = 10))]` / `#[cocogitto_schemars(length(equal = 10))]`
...
`#[validate(range(min = 1, max = 10))]` / `#[cocogitto_schemars(range(min = 1, max = 10))]`
...
`#[validate(regex = "path::to::regex")]` / `#[cocogitto_schemars(regex = "path::to::regex")]`<br />
`#[cocogitto_schemars(regex(pattern = r"^\d+$"))]`
...
`#[validate(contains = "string")]` / `#[cocogitto_schemars(contains = "string")]`
...
`#[validate(required)]` / `#[cocogitto_schemars(required)]`<br />
...
`#[cocogitto_schemars(example = "some::function")]`
...
```rs
struct Struct {
    #[cocogitto_schemars(inner(url, regex(pattern = "^https://")))]
    urls: Vec<String>,
}
```
